...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        if not CONTENT_DIR.exists():
            self.stdout.write(
                self.style.WARNING(f"Content directory not found: {CONTENT_DIR}")
            )
            return
        ...
        for path in md_files:
            ...
            if (
                published_at
                and isinstance(published_at, datetime.date)
                and not isinstance(published_at, datetime.datetime)
            ):
                ...
                published_at
                and isinstance(published_at, datetime.datetime)
                and timezone.is_naive(published_at)
            ):
