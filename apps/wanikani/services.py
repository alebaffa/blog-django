...
def _aggregate(items):
    ...
    for item in items:
        ...
        g["meaning_max_streak"] = max(
            g["meaning_max_streak"], d.get("meaning_max_streak", 0)
        )
        g["reading_max_streak"] = max(
            g["reading_max_streak"], d.get("reading_max_streak", 0)
        )
        g["count"] += 1
    ...
    for t, g in totals.items():
        ...
        g["meaning_accuracy"] = (
            round(g["meaning_correct"] / m_total * 100) if m_total else None
        )
        g["reading_accuracy"] = (
            round(g["reading_correct"] / r_total * 100) if r_total else None
        )
        total_correct = g["meaning_correct"] + g["reading_correct"]
        total_all = m_total + r_total
        g["overall_accuracy"] = (
            round(total_correct / total_all * 100) if total_all else None
        )
    ...
    overall_accuracy = (
        round(overall_correct / overall_total * 100) if overall_total else None
    )
